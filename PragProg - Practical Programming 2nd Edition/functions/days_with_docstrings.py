def get_weekday(current_weekday, days_ahead):
    ...
    current_weekday.  current_weekday is the current day of the week,
    and is in the range 1-7, indicating whether today is Sunday (1),
    Monday (2), ..., Saturday (7).
...
def get_birthday_weekday(today, current_day, birthday_day):
    """ (int, int, int) -> int

    today is the current day of the week, and is in the range 1-7,
    indicating whether today is Sunday (1), Monday (2), ..., Saturday (7).

    current_day and birth_day are both in the range 1-365.
    ...
    return get_weekday(today, days_diff)
